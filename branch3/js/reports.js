...
function fetchReport(page = 1, startDate = null, endDate = null) {
    ...
    let url = `api/sales_history.php?page=${page}&limit=${currentLimit}&include_summary=1&branch_id=${BRANCH_ID}`;
...
function exportToCSV() {
    ...
    let url = `api/sales_history.php?limit=10000&branch_id=${BRANCH_ID}`; // High limit to get all records
    if (currentStartDate && currentEndDate) {
        url += `&start_date=${currentStartDate}&end_date=${currentEndDate}`;
    }
