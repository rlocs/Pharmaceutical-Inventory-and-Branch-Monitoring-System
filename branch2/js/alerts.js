        ...
        .then(async (response) => {
            ...
            if (ct.indexOf('application/json') !== -1) {
                try {
                    ...
                    console.error('Invalid JSON from get_alerts:', text);
                    throw new Error('Invalid JSON response from alerts API');
                }
            }
            console.error('Non-JSON response from get_alerts:', response.status, text);
            throw new Error('Server returned non-JSON response');
        })
        .then(payload => {
            if (!payload || payload.success !== true) {
                console.error('Alert payload missing success flag:', payload);
                throw new Error('Failed to load alerts');
            }
            console.debug('Loaded alerts:', payload);
